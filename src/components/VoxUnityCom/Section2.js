...
const Section2 = () => {
  return (
    <div className="flex flex-col items-center w-full py-20 bg-cover" style={{ backdropFilter: 'blur(67%)', background: 'transparent' }}>
      {/* Titres */}
      <div className="text-center max-w-[80%] mb-12">
        <h1 className="text-white text-2xl md:text-5xl font-bold font-poppins">
          Votre <span className="text-[#8D1257]">Directeur de Communication</span> 
        </h1>
        <h2 className="text-white text-2xl md:text-5xl font-bold font-poppins mt-2 md:0">
          Dédié pour une Stratégie Web Réussie<span className="text-[#8D1257]">.</span>
        </h2>
      </div>
      ...
      <div className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-12 w-full max-w-[70%]">
        {data.map((section, index) => (
          <div key={index} className="space-y-6">
            <h3 className="text-white font-poppins text-lg md:text-xl font-semibold text-center">
              {section.category}
            </h3>
            {section.elements.map((element, idx) => (
              ...
              >
                <h4 className="font-poppins text-white text-md md:text-lg font-semibold mb-2">{element.title}</h4>
                <p className="text-white font-poppins text-sm md:text-base">{element.description}</p>
              </div>
            ))}
          </div>
        ))}
