    ...
    return (
        <div className="grid grid-cols-1 relative mt-24 md:max-w-[80%] mx-auto">
            <div className="flex flex-col py-8 px-8 space-y-8 items-center">
                <h2 className="text-white text-2xl md:text-5xl font-bold font-poppins mb-12 text-center">
                <span className="text-[#8D1257]">Optimisez</span> Votre Présence en Ligne avec unCommunity Manager Dédié.
                </h2>
                <div className="flex flex-col gap-4 items-start w-full">
                    {services.map((service, index) => (
                        <div key={index} className="flex flex-col w-full">
                            ...
                            >
                                <FontAwesomeIcon icon={openIndexes.includes(index) ? faTimes : faPlus} className="size-6 text-white mt-1" />
                                <h3 className="font-poppins text-white font-semibold text-xl">{service.title}</h3>
                            </div>
                            <motion.div
                                initial={false}
                            ...
                            >
                                <p className="ml-2 text-white text-base leading-relaxed">
                                    {service.description}
                                </p>
                            </motion.div>
